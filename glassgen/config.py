...
from pydantic import BaseModel, Field, Extra, ValidationError, field_validator, ValidationInfo, model_validator
...
class GlassGenConfig(BaseModel):
    ...
    def validate_duplication(self) -> 'GlassGenConfig':
        if (self.generator.event_options.duplication and 
            self.generator.event_options.duplication.enabled and 
            self.generator.event_options.duplication.key_field):
            key_field = self.generator.event_options.duplication.key_field
            if key_field not in self.schema_config:
                raise ValueError(f"key_field '{key_field}' not found in schema. Available fields: {', '.join(self.schema_config.keys())}")
        return self
